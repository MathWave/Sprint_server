...
def notificate(block, url):
    while not (block.time_start <= timezone.now() <= block.time_end and bool(block.opened)):
        block = Block.objects.get(id=block.id)
        sleep(5)
    for user in block.subscribed_users:
        if user.new_block_notification:
            send_email('New block', user.user.email, "New block is available via link: http://{}/block?id={}"
                       .format(url, block.id))
