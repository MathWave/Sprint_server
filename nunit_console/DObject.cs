...
namespace Darwin
{
    public class DObject
    {
        ...
        public DObject(string exePath, string className)
        {
            Assembly ass = Assembly.LoadFrom(exePath);
            _thisObject = Activator.CreateInstance(ass.GetType(className));
            if (_thisObject == null)
                throw new ArgumentException("Assembly or classname is wrong");
        }
